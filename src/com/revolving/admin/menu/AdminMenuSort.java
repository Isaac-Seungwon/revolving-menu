...
public class AdminMenuSort {
	...
	public void sort() {
		...
		while(true) {
			...
			Main.printMenu("메뉴 조회");
			Main.printOption("전체 조회", "카테고리별 조회", "가격 순 조회", "계절 별 조회", "이름 검색");
	...
	private boolean choiceMenu(ArrayList<Menu> list, int endIndex) {
        ...
        while (true) {
            ...
            if (input.equals("1")) {
            	...
            	if (input.matches("\\d+") && Integer.parseInt(input) > 0 && Integer.parseInt(input) <= MenuData.list.size()) {
            		AdminMenuChange adminMenuChange = new AdminMenuChange();
                	adminMenuChange.change(input);
                	return false;
            	} else {
            		System.out.println("잘못 입력하셨습니다.");
            	}
